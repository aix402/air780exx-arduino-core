...
| `CellularClientSecure::setCACert()` + `PubSubClient` | Hardware-observed | `MqttsPubSubClientCaSmoke` and the public `examples\08.Network\MqttsLoopback` sketch were flashed and runtime-verified on AIR780EPM against the anonymous EMQX public TLS MQTT broker at `broker.emqx.io:8883`. The board subscribed, published, received its loopback payload through the `PubSubClient` callback, and printed `+ARDUINO: MQTTS,PASS`. |
...
| API | Status | Notes |
| --- | --- | --- |
| `AIR780EPMSleep.lightSleep()` / `deepSleep()` | Hardware-observed for timer deep sleep wake | AIR780EPM provides board-specific sleep primitives with explicit semantics: `deepSleep()` is `SLP2`, not standby/hibernate. Current backend drives `CFUN=0` settle, `SLP2` selection, deep-sleep timer arming, and `luat_pm_force(LUAT_PM_SLEEP_MODE_DEEP)`. Board logs now show real `SLP2` entry plus RTC/timer wake on AIR780EPM. |
